...
const searchColumns = database.prepare('PRAGMA table_info(searches);').all().map((col) => col.name);
...
const candidateColumns = database.prepare('PRAGMA table_info(candidates);').all().map((col) => col.name);
...
function createMockCandidates(count, skills, location) {
...
function scoreCandidateResult(text, skills) {
    if (!skills.length)
        return 1;
...
function containsAny(value, checks) {
    if (!checks || checks.length === 0)
        return false;
    const normalized = value.toLowerCase();
    return checks.some((check) => normalized.includes(check.toLowerCase()));
}
function isValidCandidateUrl(sourceId, url) {
    try {
...
async function fetchSerpApiCandidates(skills, location) {
    ...
    const fetches = candidateSources.map(async (source) => {
        try {
            ...
            return organic
                ...
                if (containsAny(`${title} ${snippet}`, badResultKeywords))
                    return false;
                if (source.urlMustInclude && !containsAny(url, source.urlMustInclude))
                    return false;
                if (source.urlMustNotInclude && containsAny(url, source.urlMustNotInclude))
                    return false;
                if (!isValidCandidateUrl(source.id, url))
                    return false;
                return true;
            })
...
async function fetchSerpApiJobs(query, location) {
    ...
    try {
        ...
        return results.slice(0, 10).map((job) => ({
...
app.post('/api/search', async (req, res) => {
    ...
    const skillList = String(skills)
        ...
        .filter(Boolean);
    const insertColumns = ['skills', 'location'];
    const insertValues = [skills, location];
    if (hasClearance) {
    ...
    const insertSearch = database.prepare(`INSERT INTO searches (${insertColumns.join(', ')}) VALUES (${insertColumns.map(() => '?').join(', ')})`);
    ...
    const candidateInsertColumns = [];
    if (hasCandidateSearchId)
        candidateInsertColumns.push('searchId');
    ...
    if (hasCandidateSkills)
        candidateInsertColumns.push('skills');
    if (hasCandidateSummary)
        candidateInsertColumns.push('summary');
    if (hasCandidateSource)
        candidateInsertColumns.push('source');
    if (hasCandidateProfileUrl)
        candidateInsertColumns.push('profileUrl');
    if (hasCandidateResumeUrl)
        candidateInsertColumns.push('resumeUrl');
    if (hasCandidateContacted)
        candidateInsertColumns.push('contacted');
    if (hasCandidateContactedAt)
        candidateInsertColumns.push('contactedAt');
    if (hasCandidateCreatedAt)
        candidateInsertColumns.push('createdAt');
    const insertCandidate = database.prepare(`INSERT INTO candidates (${candidateInsertColumns.join(', ')}) VALUES (${candidateInsertColumns.map(() => '?').join(', ')})`);
    for (const candidate of candidates) {
        const candidateValues = [];
        if (hasCandidateSearchId)
            candidateValues.push(searchId);
        ...
        if (hasCandidateSkills)
            candidateValues.push(JSON.stringify(candidate.skills));
        if (hasCandidateSummary)
            candidateValues.push(candidate.summary);
        if (hasCandidateSource)
            candidateValues.push(candidate.source);
        if (hasCandidateProfileUrl)
            candidateValues.push(candidate.profileUrl || '');
        if (hasCandidateResumeUrl)
            candidateValues.push(candidate.resumeUrl || '');
        if (hasCandidateContacted)
            candidateValues.push(0);
        if (hasCandidateContactedAt)
            candidateValues.push(null);
        if (hasCandidateCreatedAt)
            candidateValues.push(new Date().toISOString());
...
app.get('/api/candidates/search/:searchId', (req, res) => {
    ...
    res.json(rows.map((row) => ({ ...row, skills: safeJsonParse(row.skills) })));
});
...
app.post('/api/jobs/search', async (req, res) => {
  ...
  `);
    const inserted = listings.map((listing) => {
        const result = insertListing.run(listing.title, listing.company, listing.location, listing.jobUrl, listing.description, listing.source, 'new');
