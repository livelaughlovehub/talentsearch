...
export const applicationAgent = {
  async applyToJob(jobListingId: number, config: ApplicationConfig): Promise<{
    success: boolean;
    status: string;
    message: string;
    appliedAt?: string;
  ...
  async generateCoverLetter(job: any, config: ApplicationConfig): Promise<string> {
    try {
...
Keep it under 400 words.`;
  ...
  getDefaultCoverLetter(job: any, config: ApplicationConfig): string {
...
${config.userProfile.fullName}`;
  ...
  async submitApplication(
    ...
    coverLetter: string
  ): Promise<{ success: boolean; status: string; message: string; confirmationUrl?: string; finalUrl?: string; atsType?: string }> {
    let browser;
  ...
  async applyToIndeed(page: any, browser: any, job: any, config: any, coverLetter: string) {
    try {
      ...
      const applicationType = await page.evaluate(() => {
        ...
        for (const btn of buttons) {
          ...
          const href = (btn as any).href?.toLowerCase() || '';
      ...
      for (const selector of applySelectors) {
        try {
          ...
          if (applyButton) {
            ...
            const isDisabled = await applyButton.evaluate((btn: any) => {
              return btn.disabled || 
      ...
      if (!applyButton) {
        ...
        try {
          ...
          const indeedButtonInfo = await page.evaluate(() => {
            const btn = document.querySelector('button#indeedApplyButton, button[data-testid="indeedApplyButton-test"]') as HTMLElement;
            if (btn) {
              return {
                exists: true,
      ...
      if (applyButton) {
        const buttonState = await applyButton.evaluate((btn: any) => {
          return {
  ...
  async fillFormWithAI(
    ...
    coverLetter: string,
    job: any
  ): Promise<{ success: boolean; status: string; message: string; confirmationUrl?: string }> {
    try {
        ...
        for (const mapping of fieldMapping) {
          try {
            ...
            if (fields[fieldIndex]) {
              ...
              const fieldType = await field.evaluate((el: any) => el.type || el.tagName.toLowerCase());
              ...
              if (fieldType === 'file') {
                ...
                if (value === 'true' || value === 'yes' || value === '1') {
                  const isChecked = await field.evaluate((el: any) => el.checked);
                  if (!isChecked) {
                    await field.click();
        ...
        if (hasNextButton) {
          ...
          if (nextButton && nextButton.asElement()) {
            ...
            for (const field of newFields) {
              try {
                const fieldInfo = await field.evaluate((el: any) => ({
                  type: el.type || el.tagName.toLowerCase(),
                  name: el.name || el.id || '',
                  placeholder: el.placeholder || '',
                }));
...
Return ONLY valid JSON, no other text.`;
  ...
  async fillFormRulesBased(
    ...
    for (const field of nameFields) {
      try {
        const fieldName = await field.evaluate((el: any) => el.name || el.id || '').toLowerCase();
        let value = '';
        if (fieldName.includes('first')) value = firstName;
        else if (fieldName.includes('last')) value = lastName;
        else if (fieldName.includes('full') || fieldName === 'name') value = config.userProfile.fullName;
    ...
    for (const textarea of textareas) {
      try {
        const placeholder = await textarea.evaluate((el: any) => el.placeholder || '').toLowerCase();
  ...
  async submitForm(
    ...
    const formInfo = await frame.evaluate(() => {
      ...
      const visibleForms = forms.filter((form: any) => form.offsetParent !== null);
      return {
    ...
    const errorInfo = await frame.evaluate(() => {
      ...
      return {
        ...
        buttonTexts: buttons.slice(0, 10).map((btn: any) => ({
  ...
  async checkApplicationSuccess(frame: any): Promise<boolean> {
    try {
  ...
  async applyToLinkedIn(page: any, browser: any, job: any, config: any, coverLetter: string) {
    try {
  ...
  async applyToDice(page: any, browser: any, job: any, config: any, coverLetter: string) {
    try {
  ...
  async applyToMonster(page: any, browser: any, job: any, config: any, coverLetter: string) {
    try {
  ...
  detectATSType(url: string): string | null {
  ...
  async waitForLogin(page: any, timeout: number): Promise<boolean> {
  ...
  async checkLoginRequired(page: any): Promise<boolean> {
    try {
  ...
  async detectLoginForm(page: any): Promise<{ message: string }> {
    try {
  ...
  async applyToZipRecruiter(page: any, browser: any, job: any, config: any, coverLetter: string) {
    try {
  ...
  async applyGeneric(page: any, browser: any, job: any, config: any, coverLetter: string) {
  ...
  async applyToMultipleJobs(jobIds: number[], config: ApplicationConfig): Promise<any[]> {
    const results = [];
